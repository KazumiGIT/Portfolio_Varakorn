...
- The desk terminal is **account only** (owner's decision, reaffirmed 31 Aug 2026 over Claude's 3 free questions suggestion): anonymous POSTs get the gate message and Gemini is never called. Signed in visitors get stored chat history. **Comments and vouches are NOT moderated** (owner's decision, 1 Sep 2026: "always approve and no need approval at all"). They publish the moment they are written, capped at **5 writes per person per hour** (comments and vouches share the budget, `guardRate` in store.js) so nobody can flood the site. The `approved` column stays as the take down switch: `npm run comments recent` to see what people wrote, then `hide <id>` / `hide-t <id>` (off the site, row kept) or `delete <id>` / `delete-t <id>`. `/terms` promises exactly this, so a change here means changing the house rules in the same commit.
...
- **The owner is the site moderator, signed in as `varakornm0403@gmail.com`.** `isAdmin()` in `api/_lib/supauth.js` pins it to that account's Supabase **user id** (`e2328a8b-7861-42f9-acdf-c17734a152f8`, overridable with `ADMIN_USER_IDS`) and never to the email, so nobody can claim it by signing up with an address. Signed in as him, every comment and vouch on a page carries **Hide** and **Delete**, hidden items stay visible to him marked "hidden from the site", the hourly write cap does not apply, and `POST /api/me {moderate:{kind,id,show}}` is 403 for everyone else. He cannot edit other people's words, only hide or delete them.
...
- `npm run check:account`, `check:signin` and `check:profile` are the end to end tests (each restores the test user's state so they run in any order). check:account is the broad one (self provisions a confirmed `e2e-tester@example.com` user via the admin API). **Delete that user the moment a test run ends** (admin API DELETE cascades its rows): with no approval queue its comments and vouches are live on the real pages while it exists.
