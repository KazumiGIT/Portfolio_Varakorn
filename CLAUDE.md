...
- `/account` is noindex and stays out of the sitemap. It also edits the profile: display name and photo live on the **Supabase auth user** (`user_metadata.full_name` / `avatar_url`), and `POST /api/me {sync:true}` mirrors them onto the `profiles` row that comments join against. Photos go to the public `avatars` bucket, one folder per uid, policies in `db/storage.sql`.
